...
SetupTrans();
    ...
    super.PreBeginPlay();
    //Adds some CTF rules 
    SCR = spawn(class'SmartCTFRules');
    SCR.utcompMutator = self;
    if ( Level.Game.GameRulesModifiers == None )
    	Level.Game.GameRulesModifiers = SCR;
    else    
		Level.Game.GameRulesModifiers.AddGameRules(SCR);
}
...
function PostBeginPlay()
{
    ...
    class'GrenadeAmmo'.default.InitialAmount = NumGrenadesOnSpawn;
     ...
     //comment / uncomment for unlimited trans****************************************************************
     //if(Level.Game.IsA('xCTFGame')&&bInfTrans)	
    //	class'Translauncher'.default.AmmoChargeRate = 1000000;
     //***********************************************************************************************************
}
...
function bool CheckReplacement(Actor Other, out byte bSuperRelevant)
{
    ...
    //***************Matthieu Dumont - 2006 - Replaces flags**************
    if ( xRedFlagBase(Other) != None )
      CTFBase(Other).FlagType = class'utcompvSrc.RedSmartFlag'; 
     if ( xBlueFlagBase(Other) != None )
      CTFBase(Other).FlagType = class'utcompvSrc.BlueSmartFlag'; 
...
function ModifyLogin(out string Portal, out string Options)
{
    ...
    if(Level.Game.IsA('xCTFGame')|| Level.Game.IsA('RealCTF'))
    { 
            Level.Game.ScoreBoardType="UTCompvSrc.UTComp_ScoreBoardCTF";		
    }
...
/*   else if(Level.Game.ScoreBoardType~="BonusPack.ScoreBoardLMS")
    ...
    Log("-------------XXZZXXZZ-----"$Level.Game);
    Log("-------------XXZZXXZZ---"$Level.Game.ScoreBoardType);
...
static function FillPlayInfo (PlayInfo PlayInfo)
{
    ...
    PlayInfo.AddSetting("UTComp Settings", "binfTrans", "Disable trans. limit.", 1, 1, "Check");
...
static event string GetDescriptionText(string PropName)
{
	switch (PropName)
	{
		...
		case "binfTrans":			return "Check this to disable translocator limit.";
...
defaultproperties
{
     ...
     FriendlyName="UTComp Version CTF"
     Description="A mutator for warmup, brightskins, hitsounds, and various other features tweaked for CTF."
     bNetTemporary=True
